...
fun Route.mediaRoutes(service: MediaService, repository: MediaRepository) {
    ...
    get("/api/media") {
        val dto = repository.getAll().withIndex().map { (index, entity) -> GetMedia(index, entity) }
    ...
    get("/api/media/{index}") {
        ...
        val entity = repository.getByIndex(index) ?: return@get call.respond(HttpStatusCode.NotFound)
    ...
    get("/api/media/{index_media}/chunk/{index_chunk}") {
        val indexMedia =
            call.parameters["index_media"]?.toIntOrNull() ?: return@get call.respond(HttpStatusCode.BadRequest)

        val indexChunk =
            call.parameters["index_chunk"]?.toIntOrNull() ?: return@get call.respond(HttpStatusCode.BadRequest)

        val samplesPerChunk = call.request.queryParameters["samples_per_chunk"]?.toIntOrNull()
            ?: return@get call.respond(HttpStatusCode.BadRequest)

        val entity = repository.getByIndex(indexMedia) ?: return@get call.respond(HttpStatusCode.NotFound)
    ...
    get("/api/media/{index}/cover") {
        val indexMedia = call.parameters["index"]?.toIntOrNull() ?: return@get call.respond(HttpStatusCode.BadRequest)

        val entity = repository.getByIndex(indexMedia) ?: return@get call.respond(HttpStatusCode.NotFound)

        val width =
            call.request.queryParameters["width"]?.toIntOrNull() ?: return@get call.respond(HttpStatusCode.BadRequest)

        val height =
            call.request.queryParameters["height"]?.toIntOrNull() ?: return@get call.respond(HttpStatusCode.BadRequest)

        val image = entity.cover ?: return@get call.respond(HttpStatusCode.NotFound)
